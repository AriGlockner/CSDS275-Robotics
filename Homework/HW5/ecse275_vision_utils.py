...
def hand_eye_transform(pos_cam, T_cam_world):
    ...
    """
    return np.dot(T_cam_world[:3, :3], pos_cam) + T_cam_world[:, 3]
...
def blob_images(image, keypoints, roi_size=15):
    """Returns a list of 2D pixel arrays of the regions of interest in a given image as specified by the keypoints,
    the regions are squares whose size is determined via roi size"""
    roi = []
    centroids = []
